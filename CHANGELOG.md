...
- The tarball no longer ships `.d.ts.map` files: they pointed at `src/`, which the tarball omits, so every one was a dead link. `.js.map` files stay, with the TypeScript source inlined, so stack traces land on `.ts` lines. `npm run pack:check` (the pack listing, publint and arethetypeswrong) guards the published shape in CI and as a release gate, and `exports['./package.json']` is available to tooling that reads the manifest.
