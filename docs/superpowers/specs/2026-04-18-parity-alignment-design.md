...
2. CI runs on every pull request and every push to `main`, across OS matrix `{ubuntu-latest, windows-latest, macos-latest}` × Node `{22, 24}`, and blocks merge on failure.
...
- `SECURITY.md` (separate concern, separate PR).
- Dependabot / Renovate configuration (separate concern, separate PR).
...
Delivery is split into four pull requests. Each PR MUST pass CI before the next begins. The phased ordering is load-bearing: Phase 1's CI protects Phases 2-4; Phase 2's test migration is validated before Phase 3 refactors the source; Phase 4 publishes only after all prior phases merge.

### Phase 1 — Hygiene & CI skeleton (PR #1)
...
**Triggers:** `pull_request` on any branch; `push` to `main`.
...
3. `audit` — single ubuntu-latest, Node 22.14+; runs `npm ci && npm audit --omit=dev --audit-level=high`. Failures at `high` or `critical` MUST block merge; moderate findings MUST NOT block.
...
#### 5.1.2 Phase 1 acceptance criteria

- `npm run lint`, `npm run typecheck`, `npm test`, `npm run build` all pass locally on Node 22.14+.
- A draft PR shows the CI workflow green across all matrix cells.
- No changes to `src/` files.
- No changes to test behavior; existing tests pass unmodified.

### Phase 2 — Test infrastructure migration (PR #2)
...
4. If the spike fails (typing errors, command-resolution issues, runtime bugs): **abort the `aws-sdk-client-mock` portion of Phase 2**, delete the spike, keep `tests/helpers.ts`'s hand-rolled mock as-is, and document the failure in the PR description plus a new feedback memory. Proceed with the remaining Phase 2 work.
...
**Initial integration test content:** at least one smoke test covering the happy path (create index → put vectors → query → delete index). Implementer MAY add more; the merge bar is that the suite runs successfully against a real bucket at least once before PR merge.
...
- Scripts:
  ...
  - `"test:mutate:quick"`: in Phase 2 (before the refactor) use `"stryker run --mutate src/utils.ts"`. In Phase 3, as part of the refactor PR, update this script to `"stryker run --mutate src/shared/**/*.ts"` alongside the directory rename. The script transitions atomically with the code it points at.
...
3. `npm ci`
4. `aws-actions/configure-aws-credentials` — assumes `${{ secrets.AWS_ROLE_TO_ASSUME }}` via OIDC; implementer MUST pin the latest stable major at the time of the Phase 2 PR (the action's version was not verified during spec authoring)
...
**Prerequisites NOT blocking Phase 2 PR merge:** the IAM role trust policy, the bucket, and the GitHub variable/secret MAY be configured after merge. Phase 2's merge bar is: the workflow file is syntactically valid, the gating logic has been proven locally, and the README's testing section documents the setup procedure.

#### 5.2.7 Phase 2 acceptance criteria

- All previously-passing unit tests still pass after helper swap (or hand-rolled helper retained if spike failed — documented).
- Tests split across ≥8 files; each file passes independently.
- `npm run test:integration` on a machine with `RUN_LIVE_INTEGRATION` unset exits `0` with skip message.
- `npm run test:integration` on a machine with required envs set runs at least one smoke test against real AWS successfully (implementer confirms; screenshot/log attached to PR).
- `npm run test:mutate` completes and reports ≥60% killed.
- Coverage at 80/80/80/80 with no regressions.
- Live-integration workflow file passes `actionlint` or equivalent static check.

### Phase 3 — `src/` modular refactor (PR #3)
...
#### 5.3.4 Phase 3 acceptance criteria
...
- `git diff --stat` on `test/` shows only import-path line changes.
- The refactored `src/s3-vectors.ts` is under 500 lines.
- Stryker run still passes its `low` threshold.

### Phase 4 — README, release automation, and `v0.2.0` (PR #4)
...
- README renders correctly on GitHub (headings, code blocks, badges, TOC links).
...
- After merge, pushing tag `v0.2.0` triggers the release workflow; the workflow succeeds; the package appears on npm with a visible provenance attestation; the GitHub release is auto-created with generated notes.
...
- **Live-AWS integration tests** provide the only path to verify real API behavior (response shapes, pagination, batching semantics, filter syntax, IAM enforcement). They are gated off by default because they incur AWS costs and latency and would unacceptably slow CI if run on every PR.
...
```
Phase 1 (hygiene + CI)  ──▶  Phase 2 (test infra)  ──▶  Phase 3 (src refactor)  ──▶  Phase 4 (README + release)
```

Each phase MUST fully merge before the next begins. The repository SHOULD NOT remain on an intermediate phase for longer than one calendar week to avoid drift from Project B's evolving baseline.

## 10. Implementation verification checklist

Before each phase's PR is merged, the implementer MUST confirm:

- [ ] All GitHub Action versions referenced in the PR have been re-verified against their upstream release pages within the last 24 hours.
- [ ] All npm dev-dependency versions added/changed have been verified against the npm registry.
- [ ] The relevant phase's acceptance criteria in this spec are met.
- [ ] CI is green on the final commit.
- [ ] The PR description explicitly lists any deviation from this spec with justification.

## 11. Rollback plan

- **Phase 1 rollback:** revert the PR; repo returns to pre-alignment state with no data loss (additive only).
- **Phase 2 rollback:** revert PR; unit tests continue to pass against the original helper.
- **Phase 3 rollback:** revert PR; `src/` returns to monolithic layout; test imports automatically revert with the revert.
- **Phase 4 rollback:** revert README and workflow changes; if `v0.2.0` has already been published to npm, deprecate it (`npm deprecate @farukada/aws-langchain-s3-vector-ts@0.2.0 "<reason>"`) — npm publishes are not reversible.

## 12. Open questions deferred to implementation

- Final outcome of the `aws-sdk-client-mock` spike (proceed with migration or retain hand-rolled helper) — to be recorded in the Phase 2 PR description.
