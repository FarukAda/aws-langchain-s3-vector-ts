...
| Code | Raised when |
|---|---|
| `VALIDATION` | Caller input was invalid — a mismatched count, a non-array argument, an options bag that is not an object, a bad batch size or page size, a malformed filter, a reserved metadata key or an unstorable metadata value, a malformed or repeated vector id, an unstorable vector, a string that is not well-formed UTF-16 where it would be sent to AWS, a text query that is not a string, a configuration option outside its documented set, or a `client` supplied alongside an option that would configure one. Raised before any AWS call, and before any billable embedding of the input. Errors about one element of a list carry `context.recordIndex` and, where known, `context.recordId`. |
...
| `SERVICE_UNAVAILABLE` | `InternalServerException` (500), `ServiceUnavailableException` (503), `RequestTimeoutException` (408), or the SDK's own `TimeoutError` (a timed-out, refused or reset connection). A 503 from `PutVectors` also means the batch exceeded resource capacity — `context.batchSize` says how large it was, so you can split rather than retry. |
