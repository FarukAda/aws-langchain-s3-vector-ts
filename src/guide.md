...
| `SERVICE_UNAVAILABLE` | `InternalServerException` (500), `ServiceUnavailableException` (503), `RequestTimeoutException` (408), or the SDK's own `TimeoutError` (a timed-out, reset or broken connection). A 503 from `PutVectors` also means the batch exceeded resource capacity — `context.batchSize` says how large it was, so you can split rather than retry. |
