...
The embeddings side is different: `embedDocuments`/`embedQuery` come from *your* embeddings model, and this library never retries them (the interface gives no way to know whether a failure is safe to retry). Configure retries on the embeddings client itself. A model that throws surfaces as `UNEXPECTED_ERROR`, with no `context.awsCommand`; when the model is itself built on an AWS SDK (Bedrock embeddings, say) and throws that SDK's own service exception or `TimeoutError`, `context.awsErrorName` and `context.retryable` still describe it — about the model's service, not S3 Vectors.
